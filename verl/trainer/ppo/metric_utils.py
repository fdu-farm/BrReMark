...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # Add reward breakdown metrics if available
    if hasattr(batch, 'non_tensor_batch') and batch.non_tensor_batch is not None:
        breakdown_keys = ['score', 'total', 'localization', 'answer', 'keyword', 'llm_judge', 'format', 'iou']
        for key in breakdown_keys:
            if key in batch.non_tensor_batch:
                values = batch.non_tensor_batch[key]
                if len(values) > 0:
                    metrics[f"critic/reward_breakdown/{key}/mean"] = float(np.mean(values))
                    metrics[f"critic/reward_breakdown/{key}/max"] = float(np.max(values))
                    metrics[f"critic/reward_breakdown/{key}/min"] = float(np.min(values))

                    # Backward-compat: most reward fns return `score` as total reward.
                    # Keep a `total/*` view for existing dashboards when explicit `total` is absent.
                    if key == 'score' and 'total' not in batch.non_tensor_batch:
                        metrics["critic/reward_breakdown/total/mean"] = float(np.mean(values))
                        metrics["critic/reward_breakdown/total/max"] = float(np.max(values))
                        metrics["critic/reward_breakdown/total/min"] = float(np.min(values))
