...
_MM_TRUNCATION_STATS = {}


def _log_mm_truncation_event(reason: str, n_image_tokens: int, total_expected_tokens: int):
    count = _MM_TRUNCATION_STATS.get(reason, 0) + 1
    _MM_TRUNCATION_STATS[reason] = count
    if count % 20 == 0:
        print(
            f"[WARN][mm_truncation] reason={reason}, count={count}, "
            f"n_image_tokens={n_image_tokens}, expected_image_tokens={total_expected_tokens}"
        )


def _truncate_mm_inputs_to_match_tokens(
    input_ids: torch.Tensor,
    mm_inputs: Dict[str, Any],
    processor,
) -> Dict[str, Any]:
    """
    Truncate multi_modal_inputs to match the number of image tokens in input_ids.

    When input_ids is truncated (e.g., to max_total_length), some image tokens may be
    cut off. This function adjusts pixel_values and image_grid_thw accordingly to
    prevent the "Image features and image tokens do not match" error.

    For Qwen2.5-VL, each image is represented as:
    <|vision_start|><|image_pad|>...<|image_pad|><|vision_end|>
    where the number of <|image_pad|> tokens depends on the image resolution.

    Args:
        input_ids: The (possibly truncated) input token ids
        mm_inputs: Dictionary containing pixel_values, image_grid_thw, etc.
        processor: The Qwen2VL processor

    Returns:
        Adjusted mm_inputs dictionary
    """
    if not mm_inputs or processor is None:
        return mm_inputs
    ...
    def _drop_image_fields(data: Dict[str, Any]) -> Dict[str, Any]:
        """Drop only image-related tensors while keeping potential video fields."""
        return {k: v for k, v in data.items() if k not in {"pixel_values", "image_grid_thw"}}

    image_token_id = processor.tokenizer.convert_tokens_to_ids("<|image_pad|>")

    # Count total image tokens in input_ids
    ...
    # NOTE:
    # - image tokens in input_ids correspond to merged visual tokens.
    # - pixel_values rows correspond to pre-merge patches (t * h * w).
    # So we need both cumulative merged-token counts and cumulative patch counts.
    ...
    for i in range(image_grid_thw.shape[0]):
        ...
        patches = int(t.item() * h.item() * w.item())
        tokens = patches // merge_unit
        total_expected_tokens += tokens
        total_expected_patches += patches
        cumulative_tokens.append(total_expected_tokens)
        cumulative_patches.append(total_expected_patches)

    if n_image_tokens == total_expected_tokens:
        # All image tokens are present, no truncation needed.
        return mm_inputs

    if n_image_tokens > total_expected_tokens:
        # More image tokens than features is unrecoverable without editing input_ids.
        _log_mm_truncation_event("tokens_gt_features", n_image_tokens, total_expected_tokens)
        return _drop_image_fields(mm_inputs)

    if n_image_tokens == 0:
        _log_mm_truncation_event("tokens_eq_0", n_image_tokens, total_expected_tokens)
        return _drop_image_fields(mm_inputs)

    # Find how many complete images fit in n_image_tokens
    num_complete_images = 0
    patches_to_keep = 0
    for i, cum_tokens in enumerate(cumulative_tokens):
        if cum_tokens <= n_image_tokens:
            num_complete_images = i + 1
            patches_to_keep = cumulative_patches[i]
        else:
            break

    if num_complete_images == 0:
        _log_mm_truncation_event("no_complete_image", n_image_tokens, total_expected_tokens)
        return _drop_image_fields(mm_inputs)

    # If truncation cuts through an image token block, we can't keep a partial image.
    # In that case, dropping image features entirely avoids token/feature mismatch.
    if n_image_tokens != cumulative_tokens[num_complete_images - 1]:
        _log_mm_truncation_event("partial_image_block", n_image_tokens, total_expected_tokens)
        return _drop_image_fields(mm_inputs)

    if num_complete_images < image_grid_thw.shape[0]:
        _log_mm_truncation_event("truncate_to_complete_images", n_image_tokens, total_expected_tokens)

    # Truncate to complete images only.
    new_mm_inputs = dict(mm_inputs)
    new_mm_inputs["image_grid_thw"] = image_grid_thw[:num_complete_images]
    new_mm_inputs["pixel_values"] = pixel_values[:patches_to_keep]

    return new_mm_inputs
...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    # Truncate multi_modal_inputs to match truncated input_ids
    # This prevents "Image features and image tokens do not match" error
    if processor is not None:
        for i in range(batch_size * sampling_params.n):
            mm_input_list[i] = _truncate_mm_inputs_to_match_tokens(
                input_ids=state_tensor[i, :],
                mm_inputs=mm_input_list[i],
                processor=processor,
            )
            ...
            for _ in range(n):
                if tool_name:
                    ...
                    reset_output = tool_fns.reset(
                        ...
                        extra_info=deepcopy(data_item.non_tensor_batch.get("extra_info", None)),
                        reward_model=deepcopy(data_item.non_tensor_batch.get("reward_model", None)),
                    )
